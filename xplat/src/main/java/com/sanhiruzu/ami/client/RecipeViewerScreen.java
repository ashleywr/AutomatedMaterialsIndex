...
public class RecipeViewerScreen extends Screen {
    ...
    // ── Layout constants ────────────────────────────────────────────────────
    private int guiWidth = 240; // Dynamically set in init()
    ...
    private record Tab(RecipeType<?> type, Component label, String shortLabel, List<com.sanhiruzu.ami.util.AmiRecipeHolder<?>> recipes) {
    }
    ...
    private void rebuildTabs() {
        ...
        if (index.isBuilt() && minecraft != null && minecraft.level != null) {
            Map<RecipeType<?>, List<com.sanhiruzu.ami.util.AmiRecipeHolder<?>>> grouped = new LinkedHashMap<>();
            if (focusType != null) {
                grouped.put(focusType, (List<com.sanhiruzu.ami.util.AmiRecipeHolder<?>>) (Object) index.getAllRecipesOfType(focusType));
            } else {
                List<com.sanhiruzu.ami.util.AmiRecipeHolder<?>> all = showRecipes ? index.getRecipesFor(target) : index.getUsesFor(target);
                for (var holder : all) {
                    grouped.computeIfAbsent(holder.value().getType(), k -> new ArrayList<>()).add(holder);
                }
            }
    ...
    private void drawPanel(GuiGraphics g) {
        ...
        g.fill(guiLeft + 4, contentTop, guiLeft + guiWidth - 4, contentBot, COL_PANEL_INNER);
    }
    ...
    private void drawHeader(GuiGraphics g) {
        ...
        g.drawCenteredString(font, modeLabel,
                guiLeft + guiWidth / 2, guiTop + 5, COL_TEXT_TITLE);

        // Item name as sub-label - Truncate if too long for guiWidth
        Component itemName = target.getHoverName();
        String itemStr = itemName.getString();
        int maxNameW = guiWidth - 20;
        if (font.width(itemStr) > maxNameW) {
            itemStr = font.plainSubstrByWidth(itemStr, maxNameW - 10) + "...";
        }
        g.drawCenteredString(font, itemStr,
                guiLeft + guiWidth / 2, guiTop + 5 + font.lineHeight + 2, COL_TEXT_ITEM);

        // Separator line
        g.fill(guiLeft + 4, guiTop + HEADER_H, guiLeft + guiWidth - 4, guiTop + HEADER_H + 1, COL_HEADER_LINE);
    }
    ...
    private void drawTabBar(GuiGraphics g, int mouseX, int mouseY) {
        ...
        int tabW = Math.min(guiWidth / tabs.size(), 72);
        int totalW = tabW * tabs.size();
        int startX = guiLeft + (guiWidth - totalW) / 2;
        ...
        for (int i = 0; i < tabs.size(); i++) {
            ...
            // Active tab bottom accent  Ea 2px line flush with content area
            if (active) {
    ...
    private void drawContent(GuiGraphics g, int mouseX, int mouseY) {
        ...
        int cardW = guiWidth - 8;
        int cardH = 70;
        int cardX = guiLeft + 4;

        for (int i = startIdx; i < endIdx; i++) {
            com.sanhiruzu.ami.util.AmiRecipeHolder<?> recipe = tab.recipes().get(i);
            RecipeLayout layout = RecipeDisplayHelper.getLayout(recipe, minecraft.level.registryAccess());
            if (layout == null) continue;

            int cardY = guiTop + CONTENT_Y + (i - startIdx) * 74 + (recipesPerPage == 1 ? (guiHeight - CONTENT_Y - FOOTER_H - 70) / 2 : 0);
            ...
            int rx = cardX + (cardW - 160) / 2; // Center based on standard recipe width approx 160
            ...
            for (SlotPosition slot : layout.inputs()) {
                ...
                if (!slot.alternatives().isEmpty()) {
                    String slotKey = recipe.getId().toString() + "_" + slotIdx;
            ...
            if (layout.backgroundTexture() == null) {
                ...
                if (RecipeDisplayHelper.isFurnaceType(rType)) {
                    ...
                    if (recipe.value() instanceof com.sanhiruzu.ami.index.special.FuelRecipe fr) {
                        ...
                        g.blit(new net.minecraft.resources.ResourceLocation("minecraft", "textures/gui/container/furnace.png"),
                                rx + 12, ry + 2,
                                14, 14,
                                176, 0,
                                14, 14,
    ...
    /**
     * Simple right-pointing chevron arrow (8ÁE px, centred at x,y).
     */
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollY) {
        if (tabs.isEmpty()) return super.mouseScrolled(mouseX, mouseY, scrollY);
        Tab tab = tabs.get(selectedTab);
        int tabSize = tab.recipes().size();
        if (tabSize == 0) return super.mouseScrolled(mouseX, mouseY, scrollY);
    ...
    private void doTransfer(com.sanhiruzu.ami.util.AmiRecipeHolder<?> recipe) {
