...
def github_api(method, path, token, payload=None):
    ...
    return fetch_json(f"https://api.github.com{path}", headers=headers if data is None else headers | {})
...
def find_closed_issue(owner, repo, token, title, label_name):
    issues = github_request(
        "GET",
        f"/repos/{owner}/{repo}/issues?state=closed&labels={urllib.parse.quote(label_name)}&per_page=100",
        token,
    )
    for issue in issues:
        if issue.get("title") == title:
