...
public final class FacetIndexer {
    ...
    public static FacetProfile index(Item item, ResourceLocation id, ItemStack stack) {
        ...
        if (compostables.containsKey(item)) {
    ...
    private static void applyTypeFacts(Item item, String path, EnumSet<ItemFacet> facets) {
        ...
        if (item instanceof SpawnEggItem || path.endsWith("_spawn_egg")) {
    ...
    private static void applyTagFacts(List<String> tags, EnumSet<ItemFacet> facets) {
        for (String tag : tags) {
            ...
            if (tag.startsWith("c:tools/pickaxes") || tag.startsWith("c:tools/axes")
                    ...
                    || tag.startsWith("c:tools/shears") || tag.startsWith("c:tools/fishing_rods")) {
                facets.add(ItemFacet.HARVEST_TOOL);
            }
            if (tag.startsWith("c:tools/swords") || tag.startsWith("c:weapons/swords")) {
                facets.add(ItemFacet.MELEE_WEAPON);
            }
            if (tag.startsWith("c:tools/bows") || tag.startsWith("c:tools/crossbows")
                    || tag.startsWith("c:weapons/bows") || tag.startsWith("c:weapons/crossbows")) {
                facets.add(ItemFacet.RANGED_WEAPON);
            }
            ...
            } else if (tag.startsWith("c:storage") || tag.startsWith("c:chests")
                    || tag.startsWith("c:barrels") || tag.startsWith("c:shulker_boxes")) {
                facets.add(ItemFacet.STORAGE);
            }
            if (tag.startsWith("c:dyes")) {
                facets.add(ItemFacet.INGREDIENT_DYE);
            }
            if (tag.startsWith("c:glass_blocks")) {
                facets.add(ItemFacet.GLASS_BLOCK);
            }
            if (tag.startsWith("c:glass_panes")) {
    ...
    private static List<String> collectTags(Item item) {
        ...
        holder.tags().forEach(tag -> tags.add(tag.location().toString().toLowerCase(Locale.ROOT)));
        return tags;
    }
