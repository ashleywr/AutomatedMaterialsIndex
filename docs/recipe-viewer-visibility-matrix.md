...
| Click vanilla recipe book button while AMI is visible | Hidden | Visible if loaded | Same toggle policy as Alt-V on supported container screens. |
| Click vanilla recipe book button while AMI is hidden | Visible | Suppressed if loaded | Same toggle policy as Alt-V on supported container screens. |
