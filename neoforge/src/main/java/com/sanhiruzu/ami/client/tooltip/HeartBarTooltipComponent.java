...
public final class HeartBarTooltipComponent implements TooltipComponent, ClientTooltipComponent {
    ...
    public void extractImage(Font font, int x, int y, int w, int h, GuiGraphicsExtractor g) {
        ...
        for (int i = 0; i < heartCount; i++) {
            ...
            g.blitSprite(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, HEART_CONTAINER, hx, y, HEART_SIZE, HEART_SIZE);
        }
        ...
        for (int i = 0; i < heartCount; i++) {
            ...
            g.blitSprite(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, isFull ? HEART_FULL : HEART_HALF, hx, y, HEART_SIZE, HEART_SIZE);
        }

        // Overflow label when health exceeds MAX_HEARTS * 2 HP
        if (maxHealth > MAX_HEARTS * 2) {
            int labelX = x + MAX_HEARTS * (HEART_SIZE + HEART_GAP) + 2;
            g.text(font, overflowLabel(), labelX, y + 1, AMITheme.HEART_OVERFLOW_COLOR, false);
        }

        // HP value label below hearts
        g.text(font, healthLabel(), x, y + HEART_SIZE + LABEL_TOP_PAD, AMITheme.HEART_LABEL_COLOR, false);
    }
