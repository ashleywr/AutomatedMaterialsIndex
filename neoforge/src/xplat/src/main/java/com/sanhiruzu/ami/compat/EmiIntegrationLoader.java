            ...
            for (var entry : recipesByCat.entrySet()) {
                ...
                for (Object capturedObj : entry.getValue()) {
                    try {
                        ...
                        com.sanhiruzu.ami.neoforge.AMI.LOGGER.debug("Failed to index EMI recipe", e);
                    }
                }
            }

            com.sanhiruzu.ami.neoforge.AMI.LOGGER.info("AMI/EMI bridge: indexed {} captured recipes across {} categories",
                    idCounter, recipesByCat.size());

        } catch (ClassNotFoundException ignored) {
            // EMI integration not available
        } catch (Exception e) {
            com.sanhiruzu.ami.neoforge.AMI.LOGGER.debug("AMI/EMI bridge loading failed", e);
        }
    }
}
